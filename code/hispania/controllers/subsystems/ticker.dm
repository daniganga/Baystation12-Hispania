...
/datum/controller/subsystem/ticker/Initialize(start_uptime)
	...
	build_mode_cache()
	to_world("<span class='info'><B>Bienvenido al lobby previo al juego!</B></span>")
	to_world("Por favor, configure su personaje y seleccione 'Unirse al inicio de la ronda'. El juego comenzara en [round(pregame_timeleft/10)] segundos")
...
/datum/controller/subsystem/ticker/proc/setup_tick()
	...
	if (!start_ASAP)
		...
		if (!length(ready))
			...
			to_world("<b>No hay jugadores de inicio.</b> Volviendo al lobby previo al juego.")
			return
#endif

	switch(choose_gamemode())
		if(CHOOSE_GAMEMODE_SILENT_REDO)
			log_debug("Volver a rotar silenciosamente el modo de juego...")
			return
		if(CHOOSE_GAMEMODE_RETRY)
			pregame_timeleft = 30 SECONDS
			Master.SetRunLevel(RUNLEVEL_LOBBY)
			to_world("<B>No se puede elegir el modo de juego.</B> Volviendo al lobby previo al juego para intentarlo de nuevo.")
			return
		if(CHOOSE_GAMEMODE_REVOTE)
			...
			gamemode_vote_results = null
			Master.SetRunLevel(RUNLEVEL_LOBBY)
			to_world("<B>No se puede elegir el modo de juego.</B> Volviendo al lobby previo al juego para una nueva votacion.")
			return
		if(CHOOSE_GAMEMODE_RESTART)
			to_world("<B>No se puede elegir el modo de juego.</B> Reiniciando mundo.")
	...
	spawn(0)//Forking here so we dont have to wait for this to finish
		...
		to_world("<span class='info'><B>Disfruta el juego!</B></span>")
	...
	if(!GLOB.admins.len)
		send2adminirc("La ronda ha comenzado sin administradores en linea.")
...
/datum/controller/subsystem/ticker/proc/playing_tick()
	...
	if(mode_finished && game_finished())
		...
		mode.cleanup()
		log_and_message_admins(": Todos los antagonistas han fallecido o el modo de juego ha finalizado.") //Outputs as "Event: All antagonists are deceased or the gamemode has ended."
...
/datum/controller/subsystem/ticker/proc/post_game_tick()
	switch(end_game_state)
		...
		if(END_GAME_READY_TO_END)
			...
			if (game_over)
				if(!delay_end)
					to_world("<span class='notice'><b>Reinicio debido a la destruccion de la [station_name()] en [restart_timeout/10] segundos</b></span>")

			else
				if(!delay_end)
					to_world("<span class='notice'><b>Reiniciando en [restart_timeout/10] segundos</b></span>")
			handle_tickets()
		if(END_GAME_ENDING)
		...
		if(END_GAME_DELAYED)
			...
			end_game_state = END_GAME_READY_TO_END
			log_error("Ticker llego al final de la ronda en un estado final inesperado.")
...
/datum/controller/subsystem/ticker/proc/choose_gamemode()
	...
	if(!mode_to_try)
		log_debug("No se pudo encontrar un modo de juego valido desde la configuracion o los resultados de la votacion.")
		return
	if(mode_to_try in bad_modes)
		log_debug("No se pudo iniciar el modo de juego [mode_to_try] - El modo aparece en bad_modes.")
		return
	...
	if (mode_to_try=="secret")
		...
		if(secret_force_mode != "secret") // Config option to force secret to be a specific mode.
			...
			log_debug("No se pudo iniciar el modo de juego [mode_to_try] - No hay modos ejecutables disponibles para comenzar, o todas las opciones enumeradas en modos son incorrectos.")
			return
		else
	...
	if(!istype(mode_datum))
		bad_modes += mode_to_try
		log_debug("No se pudo encontrar un modo de juego valido para [mode_to_try].")
		return
	...
	if(result)
		...
		log_debug("No se pudo iniciar el modo de juego [mode_to_try] ([mode_datum.name]) - No cumplio con los requisitos - [result]")
		return
	...
	if(mode_to_try == "secret")
		to_world("<B>El modo de juego actual es Secreto!</B>")
		...
		if (config.secret_hide_possibilities)
			message_admins("<B>Posibilidades:</B> [english_list(mode_names)]")
		else
			to_world("<B>Posibilidades:</B> [english_list(mode_names)]")
	else
		mode.announce()
	...
	if(captainless)
		for(var/mob/M in GLOB.player_list)
			if(!istype(M,/mob/new_player))
				to_chat(M, "La Comandancia no es ejercida por nadie.")
...
/datum/controller/subsystem/ticker/proc/attempt_late_antag_spawn(var/list/antag_choices)
	...
	while(antag_choices.len && antag)
		...
		if (needs_ghost)
			...
			antag_pool.Cut()
			to_world("<b>Se necesita un fantasma para engendrar en [antag.role_text].</b>\nLos fantasmas pueden entrar en el grupo antag asegurandose de que sus [antag.role_text] preferencia se establecio en alto, luego se usa el verbo alternar-agregar-antag-candidatura. Tienes 3 minutos para entrar.")
			...
			for(var/datum/mind/candidate in antag.candidates)
				if(!(candidate in antag_pool))
					...
					log_debug("[candidate.key] no estaba en el grupo antag y no se pudo seleccionar.")
		else
			antag.update_current_antag_max(mode)
			antag.build_candidate_list(mode, needs_ghost)
			for(var/datum/mind/candidate in antag.candidates)
				if(isghostmind(candidate))
					antag.candidates -= candidate
					log_debug("[candidate.key] es un fantasma y no se puede seleccionar.")
		if(length(antag.candidates) >= antag.initial_spawn_req)
			...
			if(antag.initial_spawn_req > 1)
				to_world("No se pudo encontrar suficiente [antag.role_text_plural].")

			else
				to_world("No se pudo encontrar un [antag.role_text].")

			antag_choices -= antag
			if(length(antag_choices))
				antag = antag_choices[1]
				if(antag)
					to_world("Intentando engendrar [antag.role_text_plural].")
	return 0
...
/datum/controller/subsystem/ticker/proc/notify_delay()
	if(!delay_notified)
		to_world("<span class='notice'><b>Un administrador ha retrasado el final de la ronda.</b></span>")
	delay_notified = 1
...
/datum/controller/subsystem/ticker/proc/handle_tickets()
	for(var/datum/ticket/ticket in tickets)
		if(ticket.is_active())
			if(!delay_notified)
				message_staff("<span class='warning'><b>Reinicio retrasado automaticamente debido a tickets activos.</b></span>")
			notify_delay()
			end_game_state = END_GAME_AWAITING_TICKETS
			return
	message_staff("<span class='warning'><b>No quedan tickets activos, reiniciando en [restart_timeout/10] segundos si un administrador no ha retrasado el final de la ronda.</b></span>")
	end_game_state = END_GAME_ENDING

/datum/controller/subsystem/ticker/proc/declare_completion()
	to_world("<br><br><br><H1>La ronda de [mode.name] ha terminado!</H1>")
	for(var/client/C)
		if(!C.credits)
			C.RollCredits()
	...
	for(var/mob/living/silicon/ai/aiPlayer in SSmobs.mob_list)
		...
		to_world("<b>[aiPlayer.name][show_ai_key ? " (played by [aiPlayer.key])" : ""] leyes en el [aiPlayer.stat == 2 ? "time of their deactivation" : "end of round"] fueron:</b>")
		aiPlayer.show_laws(1)

		if (aiPlayer.connected_robots.len)
			var/minions = "<b>[aiPlayer.name] secuaces leales eran:</b>"
	...
	for (var/mob/living/silicon/robot/robo in SSmobs.mob_list)
		...
		if (!robo.connected_ai)
			var/show_robot_key = robo.get_preference_value(/datum/client_preference/show_ckey_credits) == GLOB.PREF_SHOW
			to_world("<b>[robo.name][show_robot_key ? " (played by [robo.key])" : ""] leyes individuales en el [robo.stat == 2 ? "time of their deactivation" : "end of round"] fueron:</b>")
	...
	log_game("Los antagonistas en la ronda fueron...")
