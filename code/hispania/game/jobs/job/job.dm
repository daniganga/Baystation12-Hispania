...
/datum/job/New()
	...
	if(!hud_icon)
		hud_icon = "hud[ckey(title_english)]"
	...
	if(H.psi)
		...
		if(give_psionic_implant_on_join)
			...
			if(affected)
				...
				imp.part = affected
			to_chat(H, SPAN_DANGER("Como psionico registrado, esta equipado con un implante de control de amortiguacion psionica. El uso de psi-power mientras el implante esta activo dará como resultado descargas neuronales y se informara su violacion."))
...
/datum/job/proc/setup_account(var/mob/living/carbon/human/H)
	...
	var/datum/money_account/M = create_account("[H.real_name]'s cuenta", H.real_name, money_amount)
	if(H.mind)
		var/remembered_info = ""
		remembered_info += "<b>Su numero de cuenta es:</b> #[M.account_number]<br>"
		remembered_info += "<b>El pin de tu cuenta es:</b> [M.remote_access_pin]<br>"
		remembered_info += "<b>Los fondos de su cuenta son:</b> [GLOB.using_map.local_currency_name_short][M.money]<br>"

		if(M.transaction_log.len)
			var/datum/transaction/T = M.transaction_log[1]
			remembered_info += "<b>Tu cuenta fue creada:</b> [T.time], [T.date] a [T.get_source_name()]<br>"
...
/datum/job/proc/is_restricted(var/datum/preferences/prefs, var/feedback)
	...
	if(!isnull(allowed_branches) && (!prefs.branches[title] || !is_branch_allowed(prefs.branches[title])))
		to_chat(feedback, "<span class='boldannounce'>Rama de servicio incorrecta para [title]. Las sucursales validas son: [get_branches()].</span>")
		return TRUE

	if(!isnull(allowed_ranks) && (!prefs.ranks[title] || !is_rank_allowed(prefs.branches[title], prefs.ranks[title])))
		to_chat(feedback, "<span class='boldannounce'>Rango incorrecto para [title]. Rangos validos en [prefs.branches[title]] son: [get_ranks(prefs.branches[title])].</span>")
		return TRUE

	S = all_species[prefs.species]
	if(!is_species_allowed(S))
		to_chat(feedback, "<span class='boldannounce'>Especies restringidas, [S], por [title].</span>")
		return TRUE

	if(LAZYACCESS(minimum_character_age, S.get_bodytype()) && (prefs.age < minimum_character_age[S.get_bodytype()]))
		to_chat(feedback, "<span class='boldannounce'>No lo suficientemente viejo. La edad minima del personaje es [minimum_character_age[S.get_bodytype()]].</span>")
		return TRUE

	if(!S.check_background(src, prefs))
		to_chat(feedback, "<span class='boldannounce'>Antecedente incompatible para [title].</span>")
		return TRUE
